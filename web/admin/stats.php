...
$sql_where =  $id_user ? "and data.id_user='$id_user' " : '';
$sql_where .=  $id_group ? "and data.id_group='$id_group' " : '';

$sql_where2 =  $id_user ? "and data_old.id_user='$id_user' " : '';
$sql_where2 .=  $id_group ? "and data_old.id_group='$id_group' " : '';

if ($level) {
	switch ($level) {
	case 1:
		$db->sql_query("select sum(data_old.incoming) as \"incoming\", sum(data_old.outgoing) as \"outgoing\", data_old.id_user, sum(data_old.cost) as \"cost\", users.login from data_old inner join users on data_old.id_user=users.id where (data_old.log_time between '$time_from' and '$time_to') $sql_where2 group by data_old.id_user, users.login order by data_old.id_user");
		while ($stats2[] = $db->sql_fetchrow());
		$db->sql_query("select sum(data.incoming) as \"incoming\", sum(data.outgoing) as \"outgoing\", data.id_user, sum(data.cost) as \"cost\", users.login from data inner join users on data.id_user=users.id where (data.log_time between '$time_from' and '$time_to') $sql_where group by data.id_user, users.login order by data.id_user");
		break;
	case 2:
		$db->sql_query("select sum(data_old.incoming) as \"incoming\", sum(data_old.outgoing) as \"outgoing\", data_old.id_group, sum(data_old.cost) as \"cost\", routes_groups.group_name from data_old inner join routes_groups on data_old.id_group=routes_groups.id where (data_old.log_time between '$time_from' and '$time_to') $sql_where2 group by data_old.id_group, routes_groups.group_name order by data_old.id_group");
		while ($stats2[] = $db->sql_fetchrow());
                $db->sql_query("select sum(data.incoming) as \"incoming\", sum(data.outgoing) as \"outgoing\", data.id_group, sum(data.cost) as \"cost\", routes_groups.group_name from data inner join routes_groups on data.id_group=routes_groups.id where (data.log_time between '$time_from' and '$time_to') $sql_where group by data.id_group, routes_groups.group_name order by data.id_group");
		break;
	default:
		$level=1;
